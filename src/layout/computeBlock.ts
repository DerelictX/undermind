...
function mergeBlock(nd: pos_node & { v: number }, queMin: FlagHeapUtils<pos_node>,
                    unionFind: UnionFind, sizeMap: Record<number, number>, posSeqMap: Record<number, number[]>, roomWalkable: CostMatrix,
    ...
    const nearCntMap: Record<number, number> = {}
    let allNearCnt = 0

    //搜索附近的块
    posSeqMap[pos].forEach(e => {
        const y = e % 50;
        const x = Math.round((e - y) / 50);//Math.round
        forNear(roomWalkable, (x, y, val) => {
    ...
    for (let currentPos in nearCntMap) {
        const currentRatio = nearCntMap[currentPos] / Math.sqrt(Math.min(sizeMap[currentPos], nd.w))//实际/期望
    ...
    if (minSize > minPlaneCnt)
        targetBlockPutAbleCnt = getBlockPutAbleCnt(roomWalkable, visited, queMin, unionFind, targetPos, putAbleCacheMap, allCacheMap)[0].length
    if (nd.w > minPlaneCnt)
        ndkBlockPutAbleCnt = getBlockPutAbleCnt(roomWalkable, visited, queMin, unionFind, nd.v, putAbleCacheMap, allCacheMap)[0].length
    ...
    if (targetPos && Math.max(targetBlockPutAbleCnt, ndkBlockPutAbleCnt) < minPlaneCnt) {//&&(maxRatio-Math.sqrt(cnt)/20>=0||(nearRatio>0.7&&nd.k<100))
        ...
        return {w: nd.w, x: nd.x, y: nd.y, v: nd.v};
    }
}
...
function computeBlock(roomName: string) {
    ...
    queMinBlock.whileNoEmpty(nd => {
        const newNode: undefined | pos_node & { v: number }
            = mergeBlock(nd, queMin, unionFind, sizeMap, posSeqMap, roomWalkable, putAbleCacheMap, allCacheMap)
        if (newNode) queMinBlock.push(newNode);
    })
    global.layout = {
        unionFind: unionFind, sizeMap: sizeMap,
        roomWalkable: roomWalkable, nearWall: nearWall,
        putAbleCacheMap: putAbleCacheMap, allCacheMap: allCacheMap
    }
}