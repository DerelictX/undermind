...
function computeBlock(roomName: string) {
    const queMin = new FlagHeapUtils<_node>()
    ...
    // 对距离的格子插入到队列 ，作为分开的顺序
    const startPoint = new FlagHeapUtils<_node>()
    ...
    visited = new PathFinder.CostMatrix()
    const queMinBlock = new FlagHeapUtils<_node & { v: number }>()
    ...
    let putAbleCacheMap: Record<number, _node[]> = {}
    let allCacheMap: Record<number, _node[]> = {}
    // let i = 0
    // 合并小块成大块的
    const queMin2 = new FlagHeapUtils<_node>()
