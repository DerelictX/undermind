export const link_run = function(room: Room){
    const links_in = room.memory.structures.links.ins.map(id => Game.getObjectById(id))
    const links_out = room.memory.structures.links.outs.map(id => Game.getObjectById(id))
    const link_nexus = room.memory.structures.links.nexus.map(id => Game.getObjectById(id))
